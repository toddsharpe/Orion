...
installed (`apt install graphviz`, `winget install graphviz`; `GRAPHVIZ_DOT` names the `dot` to use
when it is not on the `PATH`); without it the `.dot` is still written and the compile warns once.
