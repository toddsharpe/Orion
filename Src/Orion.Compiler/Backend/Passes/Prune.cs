	...
	internal static class Prune
	{
		...
		private static HashSet<FunctionSymbol> Reachable(SymbolTable root)
		{
			...
			while (work.Count > 0)
			{
				...
				//A function reached through a FunctionRefSymbol or a literal handle may sit outside the table, so it has no graph node to walk.
		...
		private static HashSet<TypeSymbol> LiveTypes(SymbolTable root, IEnumerable<SourceFunctionSymbol> live)
		{
