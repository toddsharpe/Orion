...
namespace Orion.Backend.Passes
{
	...
	{
		//Where a symbol's writes are, or the function itself when no symbol is named.
		private static InputRegion Declared(SourceFunctionSymbol func, DataSymbol symbol) =>
			symbol == null
				? func.Located()
				: func.Tacs.OfType<ResultTac>().Where(t => t.Result == symbol).FirstOrDefault(t => t.Region != null)?.Region ?? InputRegion.None;
		...
		internal static void UniqueLocals(SourceFunctionSymbol func, List<Message> messages, IReadOnlySet<string> functions = null)
		{
			...
			{
				int n = 2;
				while (!taken.Add($"{symbol.Name}_{n}"))
					n++;

				LocalDataSymbol patched = symbol with { Name = $"{symbol.Name}_{n}" };
				...
				messages.Trace($"PrePass: {func.Name} local {symbol.Name} renamed {patched.Name} ({why})", Declared(func, symbol));
			}
			...
			foreach (IGrouping<string, LocalDataSymbol> group in locals.GroupBy(i => i.Name))
			{
				...
				foreach (LocalDataSymbol symbol in shadows ? group : group.Skip(1))
					Rename(symbol, shadows ? "shadows a function" : "sibling scope reuse");
			}
		}
		...
		internal static void Statics(SourceFunctionSymbol func, List<Message> messages)
		{
			foreach (LocalDataSymbol symbol in Declares(func))
			{
				...
				messages.Trace($"PrePass: static {func.Name}.{symbol.Name} -> global {patched.Name}", Declared(func, symbol));
			}
		}
		...
		internal static void OutParams(SourceFunctionSymbol func, List<Message> messages)
		{
			...
			if (outs.Count != 0)
				messages.Trace($"PrePass: {func.Name} returns {outs.Count} out param(s) by value", Declared(func, null));

			//The function's own returns carry its out params; a call to a function with some receives them the same way.
			Replace(func, func.Tacs.Select<Tac, Tac>(i =>
			{
				switch (i)
				{
					case ReturnVoidTac when outs.Count != 0:
						return new MultiReturnTac(outs);

					case ReturnSymTac ret when outs.Count != 0:
						return new MultiReturnTac([ret.Symbol, .. outs]);

					case CallTac call when call.Function is not BuiltinFunctionSymbol { IsExtern: true }:
