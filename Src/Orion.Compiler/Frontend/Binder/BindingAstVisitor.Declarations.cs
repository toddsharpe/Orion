...
namespace Orion.Frontend.Binder
{
	...
	internal static partial class BindingAstVisitor
	{
		...
		public static void Visit(BindContext ctx, Parameter param)
		{
			...
			TypeSymbol type = ResolveDeclaringType(ctx, param.TypeName, $"Parameter {param.Name}", param.Region);
			...
			else
			{
				...
				ParamDataSymbol port = new ParamDataSymbol(param.Name, type, param.Directive switch
				{
				...
				port.Borrowed = param.IsConst && Surface.IsCollection(type);

				if (param.Default != null)
				{
					switch (param.Directive)
					{
						case ParamDirective.State or ParamDirective.Output:
						{
							...
							{
								port.Init = init;
								port.InitValue = Seed(param.Default, type);
							}
							else
								ctx.Messages.Add(new Message(
						...
						case ParamDirective.Input or ParamDirective.Prev:
						{
		...
		public static void Visit(BindContext ctx, Extern ext)
		{
			...
			//An extern takes named types only: no array, span or generic form, so a plain lookup is the whole resolution.
		...
		private static void Define(BindContext ctx, Struct @struct)
		{
			...
			structSymbol.Fields.AddRange(@struct.Fields.Select(i => new Field(i.Name, FieldType(ctx, @struct, i))));
			structSymbol.Hosted = BuildAssembly.Complete(structSymbol);
		}

		private static TypeSymbol FieldType(BindContext ctx, Struct @struct, StructField field)
		{
			SymbolTable current = ctx.Scoper.Peek();
		...
		//The declared type of a scalar constant, or null once its absence is reported.
		private static TypeSymbol ConstantType(BindContext ctx, SymbolTable current, TypeName typeName, string name, InputRegion region)
		{
			if (current.TryGet(typeName.Name, out TypeSymbol type))
				return type;

			if (typeName.Measure != null)
				return Measured(ctx, current, typeName, $"Constant {name}", region);
		...
		private static void DeclareConstant(BindContext ctx, TypeName typeName, string name, object value, InputRegion region)
		{
			SymbolTable current = ctx.Scoper.Peek();
			TypeSymbol type = ConstantType(ctx, current, typeName, name, region);
			if (type == null)
				return;
			...
			current.AddConst(name, InternLiteral(current, raw, type));
		}
		...
		public static void Visit(BindContext ctx, Const @const)
		{
			...
			DeclareConstant(ctx, @const.TypeName, @const.Name, folded, @const.Region);
		}
		...
		public static void Visit(BindContext ctx, Enum @enum)
		{
			...
			{
				EnumTypeSymbol enumSymbol = new EnumTypeSymbol(@enum.Name, @enum.Members.Select(i => new Member(i.Name, i.Value)).ToList())
					with { IsBuild = @enum.IsBuild, IsExport = @enum.IsExport, Region = @enum.Region };
