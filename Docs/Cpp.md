...
program's translation unit. A unit includes one program's companion; two units that each define a
type two programs share, alike, is what the one-definition rule allows, so an image built from several
programs needs nothing more.
